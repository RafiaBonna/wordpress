...
<section class="blog-area inarea-blog-2-column-area three">
	<div class="container">
		<div class="row">
			<?php 
                $jewellery_boutique_paged_sidebar_setting = get_theme_mod('jewellery_boutique_paged_sidebar_setting','1');
                $jewellery_boutique_sidebar_position = get_theme_mod('jewellery_boutique_sidebar_position', 'right');
                $jewellery_boutique_content_class = ($jewellery_boutique_paged_sidebar_setting == '') ? 'col-lg-12' : 'col-lg-8';
	            ...
	            <?php 
	                if ( class_exists( 'woocommerce' ) ) {
	                    if( is_account_page() || is_cart() || is_checkout() ) {
	                        echo '<div class="' . esc_attr($jewellery_boutique_content_class . ' ' . $jewellery_boutique_content_order_class) . '">';
	                    }
	                    else{
	                        echo '<div class="' . esc_attr($jewellery_boutique_content_class . ' ' . $jewellery_boutique_content_order_class) . '">';
	                    }
				...
				<?php 	if( have_posts()) : the_post(); ?>
					<article class="post-items post-single">
						...
						<div class="post-content">
							<?php the_content();
							wp_link_pages( array(
				                'before' => '<div class="page-links">' . __( 'Pages:', 'jewellery-boutique' ),
				                'after'  => '</div>',
				            ) ); ?>
						</div>
					</article>
			...
			<?php if( $jewellery_boutique_paged_sidebar_setting != '') { ?> 
                <?php get_sidebar(); ?>
            <?php } ?>
		</div>
	</div>
